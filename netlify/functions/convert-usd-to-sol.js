...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { usdAmount, solAmount } = JSON.parse(event.body);
    if (!usdAmount || !solAmount || usdAmount <= 0 || solAmount <= 0) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Valid usdAmount and solAmount required' }) };
    }
    ...
    if ((wallet.stockBalance || 0) < usdAmount) {
    ...
    console.error('convert-usd-to-sol error:', error);
