...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { ticker, shares, price, proceeds } = JSON.parse(event.body);
    if (!ticker || !shares || !price || !proceeds) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'ticker, shares, price, proceeds required' }) };
    }
    ...
    if (!holdings[ticker] || holdings[ticker].shares < shares) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Not enough shares' }) };
    }

    const newShares = holdings[ticker].shares - shares;
    if (newShares === 0) {
      delete holdings[ticker];
    } else {
      holdings[ticker] = { ...holdings[ticker], shares: newShares };
    }

    const newStockBalance = (wallet.stockBalance || 0) + proceeds;
